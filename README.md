...
- 回归脚本：`do/04_regression.do`（输出 `output/tables/reg_scale_on_rent_stata.csv`）
- 绘图脚本：`do/05_figures.do`（输出 `output/figures/rent_vs_scale_stata.png`）
- 一键运行：在 `do/` 目录执行 `do run_stata.do`，或命令行执行：
